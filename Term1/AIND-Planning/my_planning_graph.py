...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for a in self.all_actions:
            # generate action PgNode
            nodeATmp = PgNode_a(a)
            # we need to check if action a is reachable from S_i nodes
            if nodeATmp.prenodes.issubset(sLevels):
                # add this action PgNode
                pgNodeActions.add(nodeATmp)
                # now, we need to link parent and children nodes
                for l in sLevels:
                    l.children.add(nodeATmp)
                    nodeATmp.parents.add(l)
        # Finally, add this action nodes
        self.a_levels.append(pgNodeActions)
    ...
    def add_literal_level(self, level):
        ...
        for a in self.a_levels[level - 1]:
            # loop over effect nodes of action a
            for l in a.effnodes:
                ...
                # add each reachable effect S_i+1 node
                pgNodeLevels.add(l)
                # link children and parent
                l.parents.add(a)
                a.children.add(l)
        # append reachable S nodes
        self.s_levels.append(pgNodeLevels)
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        :return: bool
        """
        # similar to competing_needs_mutex
    ...
    def h_levelsum(self) -> int:
        ...
        sLevels = self.s_levels
        for g in goals:
            goalNode = PgNode_s(g, True)
            # loop over all levels and test if the goalNode is in one of the nodes
            zipsLevels = enumerate(sLevels)
            for l, n in zipsLevels:
                if goalNode in n:
                    level_sum += l
                    break
        return level_sum
